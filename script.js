// 原 JS 代码完全保留，仅修改了设置面板的保存方式（移除确定按钮，改为自动保存）以及 refreshJournalOnShow 函数。
// 下面是完整的 JS 代码，其中设置部分已改为自动保存。
...
let clearJournalData = JSON.parse(localStorage.getItem('clearJournalData') || 'false');   // 新增：是否同时删除日记数据
...
// 移除原有的保存设置按钮相关代码（原 saveSettingsBtn 已不存在）
...
function onJournalDragEnd(e) {
    ...
    document.removeEventListener('mousemove', onJournalDragMove);
...
function refreshJournalOnShow() {
    // 修复：确保导入数据后日记本立即显示
    if (journalSection && journalSection.classList.contains('active')) {
        // 如果当前正在打开日记本视图，强制返回书架视图并刷新（避免打开状态的数据不一致）
        if (notebookView && notebookView.style.display === 'block') {
            closeNotebookDiary();  // 会自动调用 renderBookshelfUI
