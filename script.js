...
const deleteTimerRecordHeader = document.getElementById('delete-timer-record-header');
...
let dragCloneStartLeft = null, dragCloneStartTop = null;
...
function createDragClone(element) {
  ...
  const origIconContainer = element.querySelector('.icon-container');
  const cloneIconContainer = clone.querySelector('.icon-container');
  if (origIconContainer && cloneIconContainer) {
    ...
    cloneIconContainer.style.minWidth = origStyles.minWidth;
    cloneIconContainer.style.minHeight = origStyles.minHeight;
    cloneIconContainer.style.display = origStyles.display;
    cloneIconContainer.style.flex = origStyles.flex;
    ...
    const origIcon = origIconContainer.querySelector('.emoji-icon, img');
    const cloneIcon = cloneIconContainer.querySelector('.emoji-icon, img');
    if (origIcon && cloneIcon) {
      const origIconStyles = window.getComputedStyle(origIcon);
      if (origIcon.tagName === 'IMG') {
        cloneIcon.style.width = origIcon.clientWidth + 'px';
        cloneIcon.style.height = origIcon.clientHeight + 'px';
        cloneIcon.style.objectFit = origIconStyles.objectFit;
      } else {
        cloneIcon.style.fontSize = origIconStyles.fontSize;
        cloneIcon.style.lineHeight = origIconStyles.lineHeight;
        cloneIcon.style.width = 'auto';
        cloneIcon.style.height = 'auto';
      }
...
function removeDragClone() {
  ...
  dragCloneStartLeft = null;
  dragCloneStartTop = null;
}
...
function updateClonePositionByOffset(clientX, clientY) {
  if (!dragClone || dragCloneStartLeft === null || dragCloneStartTop === null) return;
  const dx = clientX - dragStartX;
  const dy = clientY - dragStartY;
  dragClone.style.left = (dragCloneStartLeft + dx) + 'px';
  dragClone.style.top = (dragCloneStartTop + dy) + 'px';
}
...
function onDragMove(e) {
  ...
  updateClonePositionByOffset(clientX, clientY);

  refreshDragOrder(clientX, clientY);
...
function startDragAfterLongPress(element, clientX, clientY) {
  if (!editMode) return;
  ...
  dragLongPressTimer = setTimeout(() => {
    ...
    dragStartX = clientX;
    dragStartY = clientY;
    dragStartIndex = Array.from(element.parentNode.children).indexOf(element);
    isDraggingCard = true;
    dragClone = createDragClone(element);
    const rect = element.getBoundingClientRect();
    dragCloneStartLeft = rect.left;
    dragCloneStartTop = rect.top;
    if (dragClone) {
...
async function renderPunchList(forceRender = false) {
  ...
  for (const p of sorted) {
    ...
    li.addEventListener('touchstart', function(e) {
      ...
      if (editMode) {
        ...
        dragLongPressTimer = setTimeout(() => {
          ...
          startDragAfterLongPress(this, touch.clientX, touch.clientY);
        }, 400);
        this.touchStartX = e.touches[0].clientX;
        this.touchStartY = e.touches[0].clientY;
        this.isMoving = false;
        this.addEventListener('touchmove', cancelLongPressOnMove);
        this.addEventListener('touchend', cancelLongPressOnEnd);
        this.addEventListener('touchcancel', cancelLongPressOnEnd);
      } else {
    ...
    li.addEventListener('touchmove', function(e) {
      if (editMode) {
        if (dragLongPressTimer) {
          if (this.touchStartX && (Math.abs(e.touches[0].clientX - this.touchStartX) > 10 ||
    ...
    li.addEventListener('touchend', function(e) {
      if (editMode) {
        ...
        this.removeEventListener('touchmove', cancelLongPressOnMove);
        this.removeEventListener('touchend', cancelLongPressOnEnd);
        this.removeEventListener('touchcancel', cancelLongPressOnEnd);
        return;
      }
    ...
    li.addEventListener('mousedown', function(e) {
      ...
      if (editMode) {
        if (dragLongPressTimer) clearTimeout(dragLongPressTimer);
        dragLongPressTimer = setTimeout(() => {
          startDragAfterLongPress(this, e.clientX, e.clientY);
        }, 400);
        this.mouseStartX = e.clientX;
        this.mouseStartY = e.clientY;
        this.mouseMoved = false;
        const onMouseMoveWhileWaiting = (moveEvent) => {
          if (Math.abs(moveEvent.clientX - this.mouseStartX) > 10 ||
              Math.abs(moveEvent.clientY - this.mouseStartY) > 10) {
            clearTimeout(dragLongPressTimer);
            dragLongPressTimer = null;
...
function cancelLongPressOnMove(e) {
  if (this.dragLongPressTimer && this.touchStartX && 
      (Math.abs(e.touches[0].clientX - this.touchStartX) > 10 ||
       Math.abs(e.touches[0].clientY - this.touchStartY) > 10)) {
...
function cancelLongPressOnEnd(e) {
      ...
      punchItemElement.innerHTML = `
        <div class="day-punch-item-content">
          ...
          <div class="day-punch-item-info">
            <div class="day-punch-item-name">${escapeHtml(item.name)}</div>
...
if (deleteTimerRecordHeader) {
  deleteTimerRecordHeader.onclick = () => {
    if (!currentEditingParentId) return;
...
function mergeConsecutiveSameNameSlots(slots) {
  ...
  for (const slot of slots) {
    if (!currentGroup) {
      currentGroup = { ...slot, subSlots: [{ start: slot.start, end: slot.end, durationMs: slot.durationMs, parentId: slot.parentId, name: slot.name, isRecorded: slot.isRecorded }] };
    } else if (Math.abs(currentGroup.end - slot.start) < 0.01 && currentGroup.name === slot.name && currentGroup.isRecorded === slot.isRecorded && currentGroup.color === slot.color) {
      currentGroup.end = slot.end;
      currentGroup.durationMs += slot.durationMs;
      currentGroup.subSlots.push({ start: slot.start, end: slot.end, durationMs: slot.durationMs, parentId: slot.parentId, name: slot.name, isRecorded: slot.isRecorded });
    } else {
...
function renderTimeList(container, date) {
  ...
  const mergedSlots = mergeConsecutiveSameNameSlots(timeSlots);
  ...
  if (mergedSlots.length === 0) {
    ...
    mergedSlots.forEach(slot => {
      ...
      slotItem.innerHTML = `
        <div style="width: 100%;">
          <div class="slot-header">
            ...
            <span class="slot-name" style="color: ${color};">${escapeHtml(slot.name)}</span>
      ...
      slotItem.addEventListener('click', (e) => {
        e.stopPropagation();
        if (slot.subSlots && slot.subSlots.length > 1) {
          ...
          slot.subSlots.forEach((sub, idx) => {
            const subStartStr = formatHourFloat(sub.start);
            const subEndStr = formatHourFloat(sub.end);
            const subDuration = formatDurationPrecise(sub.durationMs);
            const menuItem = document.createElement('div');
            menuItem.textContent = `${subStartStr} - ${subEndStr} (${subDuration})`;
            ...
            menuItem.addEventListener('click', (ev) => {
              ev.stopPropagation();
              if (sub.parentId) {
                openEditTimerModal(sub.parentId);
              } else {
                const startHour = Math.floor(sub.start);
                const startMinute = Math.round((sub.start - startHour) * 60);
                const endHour = Math.floor(sub.end);
                const endMinute = Math.round((sub.end - endHour) * 60);
        ...
        } else if (slot.subSlots && slot.subSlots.length === 1) {
          const sub = slot.subSlots[0];
          if (sub.parentId) {
            openEditTimerModal(sub.parentId);
...
document.addEventListener('visibilitychange', function() {
  if (!document.hidden && timeSection && timeSection.classList.contains('active')) {
    const today = new Date();
    if (formatDate(currentTimeViewDate) !== formatDate(today)) {
      currentTimeViewDate = today;
      updateTimeHeader();
      renderTimeSummaryForDate(currentTimeViewDate);
    }
  }
...
let journalDragLongPressTimer = null;

let journalDragState = {
    isDragging: false,
    dragElement: null,
    startIndex: -1,
    clone: null,
...
function renderBookshelfUI() {
    ...
    booksGrid.innerHTML = books.map(book => `
        <div class="book-card" data-id="${book.id}">
            ...
            <button class="edit-book-btn" data-id="${book.id}" data-name="${escapeHtml(book.name)}" title="编辑名称"><i class="fas fa-edit"></i></button>
        </div>
    `).join('');
...
function refreshJournalOnShow() {
    if (journalSection && journalSection.classList.contains('active')) {
        if (notebookView && notebookView.style.display === 'block') {
            closeNotebookDiary();
        } else if (bookshelfView && bookshelfView.style.display !== 'none') {
            renderBookshelfUI();
        }
    }
}
